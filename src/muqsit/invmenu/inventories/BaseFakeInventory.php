...
abstract class BaseFakeInventory extends ContainerInventory{
	...
	public function onFakeBlockDataSend(Player $player) : void{
		...
		if($delay > 0){
		    $scheduler = InvMenuHandler::getRegistrant()->getScheduler();
		    $refClass = new ReflectionClass($scheduler);
		    $enabled = $refClass->getProperty("enabled");
		    $enabled->setAccessible(true);
		    if($enabled->getValue($scheduler)){
                $scheduler->scheduleDelayedTask(new DelayedFakeBlockDataNotifyTask($player, $this), $delay);
            }
		}else{
